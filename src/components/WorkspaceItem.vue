<template lang="">
  <li>
    <div
      ...
      :class="{active: workspace.id === $route.params.workspaceId}"
      ...
      <div class="actions">
        <div
          class="material-symbols-rounded"
          @click.stop="
            workspaceStore.createWorkspace({ parentId: workspace.id })
          ">
          add
        </div>
        <div
...
export default {
  ...
  created() {
    this.showChildren = this.workspaceStore.currentWorkspacePath.some(
      (workspace) => {
      return workspace.workspaceId === this.workspace.id
    })
  }
}
</script>
